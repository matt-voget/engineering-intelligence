...
def github_finder_section(view: dict, people_directory: dict | None = None) -> str:
    ...
    </div><div class="active-filter-row"><span class="active-filter-label" data-gh-active-label>Active filters:</span>
    ...
    <div class="finder-chart-grid">
      <figure class="weekly-chart finder-weekly-chart" data-github-finder-chart="pickup"><figcaption><div><strong>PR pickup time</strong><span>Weekly average from all filtered qualifying pull requests.</span></div></figcaption><div class="weekly-chart-canvas" role="img" aria-label="GitHub Finder weekly average PR pickup time chart"></div></figure>
      <figure class="weekly-chart finder-weekly-chart" data-github-finder-chart="review"><figcaption><div><strong>PR review time</strong><span>Weekly average from all filtered qualifying pull requests.</span></div></figcaption><div class="weekly-chart-canvas" role="img" aria-label="GitHub Finder weekly average PR review time chart"></div></figure>
    </div>
...
function finderCharts(){[['pickup','pickupHours'],['review','reviewHours']].forEach(([metric,field])=>{const records=filtered.filter(row=>row.type==='pull_request'&&row.merged&&row[field]!==null&&row[field]!==undefined);renderGithubFinderChart(scope.querySelector('[data-github-finder-chart="'+metric+'"] .weekly-chart-canvas'),records,record=>record.merged,record=>record[field],'GitHub Finder weekly average PR '+metric+' time');});}
function finderFilterChips(){const names={type:'Record',repository:'Repository',state:'State',author:'Author',authorTeam:'Author team',reviewer:'Reviewer',reviewerTeam:'Reviewer team',jira:'Jira link',from:'From',to:'To'},labels=Object.entries(filters).filter(([key,input])=>key!=='text'&&input.value).map(([key,input])=>(names[key]||key)+': '+(input.selectedOptions?.[0]?.text||input.value)),chips=scope.querySelector('[data-gh-filter-chips]');chips.replaceChildren(...labels.map(label=>{const chip=document.createElement('span');chip.className='filter-chip';chip.textContent=label;return chip;}));scope.querySelector('[data-gh-active-label]').hidden=!labels.length;}
