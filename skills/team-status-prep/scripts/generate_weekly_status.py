...
def github_finder_section(view: dict) -> str:
...
const columns=[['type','Record'],['repository','Repository'],['identifier','ID'],['title','Title'],['state','State'],['author','Author'],['date','Date'],['head','Head'],['base','Base'],['commits','Commits'],['reviews','Reviews'],['reviewers','Reviewers'],['prs','Pull requests'],['jira','Jira']];let visible=columns.map(c=>c[0]),page=0,sortKey='date',sortDir='desc',filtered=[];const size=100,head=table.tHead.rows[0],body=table.tBodies[0],manager=scope.querySelector('.github-column-manager'),filters=Object.fromEntries([...scope.querySelectorAll('[data-gh-filter]')].map(x=>[x.dataset.ghFilter,x]));
const e=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),date=r=>r.merged||r.updated||r.committed||r.authored||r.created||'',label=k=>columns.find(c=>c[0]===k)[1],cell=(r,k)=>{if(k==='identifier'||k==='title')return '<a href="'+e(r.url)+'" target="_blank" rel="noreferrer">'+e(r[k])+'</a>';if(k==='type')return r.type==='pull_request'?'Pull request':'Commit';if(k==='state')return e((r.draft?'draft · ':'')+(r.state||'—'));if(k==='date')return date(r)?'<time datetime="'+e(date(r))+'">'+e(date(r).slice(0,10))+'</time>':'—';if(k==='reviewers'||k==='prs')return e((r[k]||[]).join(', ')||'—');if(k==='jira')return r.jira.length?r.jira.map(j=>'<a href="'+e(r.jiraUrls[j])+'" target="_blank" rel="noreferrer">'+e(j)+'</a>').join(', '):'—';return e(r[k]??'—')};
...
function apply(){const text=filters.text.value.toLowerCase(),from=filters.from.value,to=filters.to.value;filtered=rows.filter(r=>(!filters.type.value||r.type===filters.type.value)&&(!filters.repository.value||r.repository===filters.repository.value)&&(!filters.state.value||r.state===filters.state.value)&&(!filters.author.value||r.author===filters.author.value)&&(!filters.reviewer.value||r.reviewers.includes(filters.reviewer.value))&&(!filters.jira.value||(filters.jira.value==='linked'?r.jira.length:!r.jira.length))&&(!from||date(r).slice(0,10)>=from)&&(!to||date(r).slice(0,10)<=to)&&(!text||[r.repository,r.identifier,r.title,r.author,r.state,...r.reviewers,...r.prs,...r.jira].join(' ').toLowerCase().includes(text)));filtered.sort((a,b)=>{const av=sortKey==='date'?date(a):a[sortKey],bv=sortKey==='date'?date(b):b[sortKey],c=String(av??'').localeCompare(String(bv??''),undefined,{numeric:true,sensitivity:'base'});return sortDir==='asc'?c:-c});page=0;draw()}
...
def main() -> None:
    ...
    finder_pages = (
        ...
        f'{github_finder_section(github_finder)}</section>'
    )
    body = (
        overview
        + finder_pages
