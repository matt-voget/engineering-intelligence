...
def issue_finder_section(issues: list[dict]) -> str:
    ...
    return (
        ...
        'apply; only issues currently in Done with complete cycles qualify.</span></figcaption>'
...
function renderWeeklyAverageChart(container,records,options){const points=weeklyAverage(records,options.date,options.value),unit=options.unit||'units',shortUnit=options.shortUnit||unit,digits=options.digits??1,recordLabel=options.recordLabel||'record';container.setAttribute('aria-label',options.label+': '+points.map(p=>'week of '+p.week+', '+p.average.toFixed(2)+' '+unit+', '+p.count+' '+recordLabel+(p.count===1?'':'s')).join('; '));if(!points.length){container.innerHTML='<div class="chart-empty">No qualifying records in the selected range.</div>';return;}const width=900,height=250,left=58,right=22,top=26,bottom=48,plotW=width-left-right,plotH=height-top-bottom,max=Math.max(...points.map(p=>p.average),1),x=i=>left+(points.length===1?plotW/2:i*plotW/(points.length-1)),y=v=>top+plotH-v/max*plotH;const ticks=[0,.25,.5,.75,1].map(f=>{const yy=y(max*f);return '<line class="chart-grid" x1="'+left+'" x2="'+(width-right)+'" y1="'+yy+'" y2="'+yy+'"/><text class="chart-axis" x="'+(left-8)+'" y="'+(yy+4)+'" text-anchor="end">'+(max*f).toFixed(1)+'</text>'}).join(''),path=points.map((p,i)=>(i?'L':'M')+x(i)+' '+y(p.average)).join(' '),marks=points.map((p,i)=>'<g><circle class="chart-point" cx="'+x(i)+'" cy="'+y(p.average)+'" r="5"><title>Week of '+p.week+': '+p.average.toFixed(2)+' '+unit+' across '+p.count+' '+recordLabel+(p.count===1?'':'s')+'</title></circle><text class="chart-label" x="'+x(i)+'" y="'+(y(p.average)-11)+'" text-anchor="middle">'+p.average.toFixed(digits)+shortUnit+' · n='+p.count+'</text><text class="chart-axis" x="'+x(i)+'" y="'+(height-19)+'" text-anchor="middle">'+p.week.slice(5)+'</text></g>').join('');container.innerHTML='<svg viewBox="0 0 '+width+' '+height+'" aria-hidden="true">'+ticks+'<text class="chart-axis" transform="translate(15 '+(top+plotH/2)+') rotate(-90)" text-anchor="middle">'+options.axisLabel+'</text><path class="chart-line" d="'+path+'"/>'+marks+'</svg>';}
...
document.querySelectorAll('.table-wrap').forEach(w=>{if(w.closest('.cycle-group,.pr-metric-group'))return;const issueTable=w.querySelector('.issue-finder-table'),dated=[...w.querySelectorAll('[data-date]')];if(issueTable)bindDateScope(w,()=>dated,row=>row.dataset.cycleEnded,null,'Done dates');else if(dated.length)bindDateScope(w,()=>dated,row=>row.dataset.date);});
...
document.querySelectorAll('.issue-finder-table').forEach(table=>{const scope=table.closest('.app-view'),chart=scope.querySelector('[data-issue-finder-chart]'),canvas=chart.querySelector('.weekly-chart-canvas'),toggle=chart.querySelector('[data-issue-outlier-toggle]'),note=chart.querySelector('[data-issue-outlier-note]'),rows=[...table.tBodies[0].rows].filter(row=>row.cells.length>1);let queued=false;function update(){queued=false;const records=rows.filter(row=>!['facet-hidden','attention-hidden','date-hidden','text-hidden'].some(name=>row.classList.contains(name))&&row.dataset.issueStatus.trim().toLowerCase()==='done'&&row.dataset.cycleEnded&&row.dataset.totalCycleDays!=='').map(row=>({key:row.querySelector('a')?.textContent.trim()||'Unknown',ended:row.dataset.cycleEnded,days:Number(row.dataset.totalCycleDays)})),result=extremeHighOutliers(records,record=>record.days),excluded=toggle.checked?result.outliers:[],plotted=records.filter(record=>!excluded.includes(record));if(result.fence===null)note.textContent='Extreme outlier detection requires at least four qualifying Done issues.';else if(!result.outliers.length)note.textContent='No extreme outliers detected. Rule: values above Q3 + 3×IQR ('+result.fence.toFixed(2)+'d). Only issues currently in Done qualify.';else note.textContent=(toggle.checked?'Excluded ':'Detected ')+result.outliers.length+' extreme outlier'+(result.outliers.length===1?'':'s')+' above Q3 + 3×IQR ('+result.fence.toFixed(2)+'d): '+result.outliers.map(record=>record.key+' — '+record.days.toFixed(2)+'d, Done '+record.ended).join('; ')+(toggle.checked?'.':'. Included until the option is selected.')+' Only issues currently in Done qualify.';renderWeeklyAverageChart(canvas,plotted,{date:record=>record.ended,value:record=>record.days,label:'Issue Finder weekly average completed Build Cycle Time',unit:'calendar days',shortUnit:'d',axisLabel:'Average calendar days',recordLabel:'issue'});}toggle.addEventListener('change',update);new MutationObserver(()=>{if(!queued){queued=true;requestAnimationFrame(update)}}).observe(table,{subtree:true,attributes:true,attributeFilter:['class']});update();});
