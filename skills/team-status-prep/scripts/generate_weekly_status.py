...
def issue_finder_section(issues: list[dict]) -> str:
    ...
    controls = (
        ...
        'aria-label="Search issues" placeholder="Search Jira key, title, assignee…"></div>'
...
def github_finder_section(view: dict, people_directory: dict | None = None) -> str:
    ...
    <div class="finder-search"><span aria-hidden="true">⌕</span><input type="search" data-gh-filter="text" aria-label="Search GitHub records" placeholder="Search title, SHA, author, Jira…"></div>
...
function syncIssueMultiLabels(scope){const controls=[...scope.querySelectorAll('[data-finder-multi]')],chips=[...scope.querySelectorAll('[data-active-filter-chips] .filter-chip')];let chipIndex=0;controls.forEach(control=>{const count=control.querySelectorAll('input:checked').length,total=control.querySelectorAll('input[type="checkbox"]').length,label=control.dataset.filterLabel,plural=control.dataset.filterPlural,selection=count===total?'All':count+' selected';control.querySelector('[data-multi-label]').textContent=count?selection:'All '+plural;if(count&&chips[chipIndex])chips[chipIndex++].textContent=label+': '+selection;});}
document.querySelectorAll('.issue-finder-view').forEach(scope=>{scope.querySelectorAll('[data-finder-multi]').forEach(control=>{control.addEventListener('change',()=>syncIssueMultiLabels(scope));control.querySelector('[data-multi-all]').addEventListener('click',()=>{control.querySelectorAll('input').forEach(input=>input.checked=true);control.dispatchEvent(new Event('change',{bubbles:true}));});control.querySelector('[data-multi-none]').addEventListener('click',()=>{control.querySelectorAll('input').forEach(input=>input.checked=false);control.dispatchEvent(new Event('change',{bubbles:true}));});});scope.querySelector('[data-finder-clear]').addEventListener('click',()=>syncIssueMultiLabels(scope));syncIssueMultiLabels(scope);});
