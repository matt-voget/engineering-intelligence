...
def github_finder_section(view: dict, people_directory: dict | None = None, all_teams: list[str] | None = None) -> str:
    ...
    </div><label class="outlier-control github-outlier-control"><input type="checkbox" data-gh-outlier-toggle> Exclude extreme outliers from charts</label><div class="outlier-note" data-gh-outlier-note></div><details class="outlier-details" data-gh-outlier-details><summary>Pull requests included and excluded from the graphs</summary><div class="table-wrap"><table><thead><tr><th>Pull request</th><th>Merged</th><th>Metric</th><th>Value</th><th>Graph status</th></tr></thead><tbody data-gh-outlier-rows></tbody></table></div></details><div class="active-filter-row"><span class="active-filter-label" data-gh-active-label>Active filters:</span>
...
function formatHours(value){return value<24?value.toFixed(1)+' h':(value/24).toFixed(1)+' d';}
...
function finderCharts(){const detected=[];[['coding','codingHours'],['pickup','pickupHours'],['review','reviewHours']].forEach(([metric,field])=>{const records=filtered.filter(row=>row.type==='pull_request'&&row.merged&&row[field]!==null&&row[field]!==undefined),result=extremeHighOutliers(records,row=>row[field]),outliers=new Set(result.outliers),plotted=outlierToggle.checked?records.filter(row=>!outliers.has(row)):records;result.outliers.forEach(row=>detected.push({row,metric,value:row[field],excluded:outlierToggle.checked}));renderGithubFinderChart(scope.querySelector('[data-github-finder-chart="'+metric+'"] .weekly-chart-canvas'),plotted,row=>row.merged,row=>row[field],'GitHub Finder weekly average PR '+metric+' time')});outlierNote.textContent=detected.length?(outlierToggle.checked?'Excluded ':'Detected ')+detected.length+' metric outlier'+(detected.length===1?'':'s')+' using Q3 + 3×IQR independently for each chart.':'No extreme chart outliers detected in the filtered pull requests.';outlierRows.innerHTML=detected.map(item=>'<tr><td><a href="'+e(item.row.url)+'" target="_blank" rel="noreferrer">'+e(item.row.repository+' '+item.row.identifier)+'</a></td><td>'+e(item.row.merged.slice(0,10))+'</td><td>'+e(item.metric[0].toUpperCase()+item.metric.slice(1))+'</td><td>'+e(hours(item.value))+'</td><td>'+(item.excluded?'Excluded':'Included (outlier detected)')+'</td></tr>').join('')||'<tr><td colspan="5" class="empty">No extreme outliers detected.</td></tr>'}
