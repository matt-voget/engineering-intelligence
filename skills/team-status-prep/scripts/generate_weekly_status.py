...
def issue_finder_section(issues: list[dict]) -> str:
    ...
    controls = (
        ...
        '<label>Text <input type="search" data-issue-finder-text '
        'placeholder="Jira, title, assignee…"></label>'
        '<label>Done from <input type="date" data-issue-date-from></label>'
        '<label>Done to <input type="date" data-issue-date-to></label>'
        '<label>Team <select multiple size="4" data-finder-field="issueTeam" '
        'data-finder-multi aria-label="Filter by one or more teams">'
        f'{options(teams)}</select></label>'
        '<label>Status <select multiple size="4" data-finder-field="issueStatus" '
        'data-finder-multi aria-label="Filter by one or more statuses">'
    ...
    return (
        ...
        'apply; only issues that entered Done with complete cycles qualify. Hover, '
        'focus, or select a point for its value and sample size.</span></figcaption>'
...
CSS += '''.finder-filters select,.finder-filters input{font:inherit;min-width:170px;padding:7px 9px;border:1px solid #bcc6dc;border-radius:7px;background:white}.finder-filters select[multiple]{min-height:104px}.chart-point{cursor:pointer}.chart-point:focus{outline:none;stroke:#172033;stroke-width:5}.chart-detail{min-height:22px;margin-top:4px;color:#26355c;font-size:13px;font-weight:600}.outlier-details{margin-top:8px}.outlier-details .table-wrap{margin-top:8px;max-height:260px;overflow:auto}'''
