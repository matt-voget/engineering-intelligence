...
FINDER_CHART_CSS = (
    ".finder-chart-grid{display:grid;grid-template-columns:1fr;gap:12px}"
    ".finder-chart-grid .weekly-chart{min-width:0}"
)
...
def issue_finder_section(issues: list[dict], all_teams: list[str] | None = None) -> str:
    teams = sorted(set(all_teams or []) | {item["team_name"] for item in issues}, key=str.casefold)
    statuses = sorted(
        {item.get("status") or "Unknown" for item in issues}, key=str.casefold
    )
    issue_types = sorted(
        {item.get("issue_type") or "Unknown" for item in issues}, key=str.casefold
    )
    ...
    def multi_filter(label: str, field: str, values: list[str]) -> str:
        ...
        return (
            f'<details class="multi-filter" data-finder-multi="{field}" '
            f'data-filter-label="{esc(label)}" data-filter-plural="{plural}">'
            f'<summary><span>{esc(label)}</span><strong data-multi-label>'
            f'All {plural}</strong></summary>'
            '<div class="multi-menu"><div class="multi-actions">'
            '<button type="button" data-multi-all>Select all</button>'
            '<button type="button" data-multi-none>Select none</button></div>'
            f'{choices}</div></details>'
        )

    controls = (
        '<section class="finder-filter-card"><div class="finder-search">'
        '<span aria-hidden="true">⌕</span><input type="search" data-issue-finder-text '
        'aria-label="Search issues" placeholder="Search Jira key, title, assignee…">'
        '<button type="button" data-issue-search>Search</button></div>'
        '<div class="finder-filters">'
        '<label>Done from <input type="date" data-issue-date-from></label>'
        '<label>Done to <input type="date" data-issue-date-to></label>'
        ...
        '</select></label>'
        '</div><div class="active-filter-row"><span class="active-filter-label">Active filters:</span>'
        '<div class="active-filter-chips" data-active-filter-chips></div>'
        '<button class="filter-clear" type="button" data-finder-clear>Clear all</button>'
        '<span class="date-note" data-issue-date-note></span>'
        '<span class="finder-count" aria-live="polite"></span></div></section>'
    )
    column_manager = (
    ...
    return (
        ...
        + controls
        + '<div class="finder-secondary-actions">'
        + threshold_manager
        + column_manager
        + '</div>'
        + '<figure class="weekly-chart finder-weekly-chart" data-issue-finder-chart>'
        '<figcaption><div><strong>Completed build cycle time</strong>'
        '<span>Grouped by the UTC Monday of each Done transition. All table filters '
        'apply; only issues that entered Done with complete cycles qualify.</span></div>'
        '<div class="chart-kpis"><span>Current average<strong data-chart-average>—</strong></span>'
        '<span>Completed issues<strong data-chart-sample>0</strong></span>'
        '<span>Trend<strong data-chart-trend>—</strong></span></div></figcaption>'
        '<div class="weekly-chart-canvas" role="img" aria-label="Issue Finder weekly '
        'average completed Build Cycle Time chart"></div>'
        '<label class="outlier-control"><input type="checkbox" '
        'data-issue-outlier-toggle> Exclude extreme outliers</label>'
        '<div class="outlier-note" data-issue-outlier-note></div>'
        '<details class="outlier-details" data-issue-outlier-details>'
        '<summary>Issues included and excluded from the graph</summary>'
        '<div class="table-wrap"><table><thead><tr><th>Issue</th><th>Done</th>'
        '<th>Cycle time</th><th>Graph status</th></tr></thead>'
        '<tbody data-issue-outlier-rows></tbody></table></div></details></figure>'
...
def github_finder_section(view: dict, people_directory: dict | None = None, all_teams: list[str] | None = None) -> str:
    ...
    people_by_login = {
        row["github_login"].casefold(): row.get("display_name")
        for row in people if row.get("github_login")
    }
    teams = sorted(
        set(all_teams or []) | {team for memberships in teams_by_login.values() for team in memberships},
        key=str.casefold,
    )
    ...
    def multi_filter(label: str, field: str, values: list[tuple[str, str]]) -> str:
        choices = "".join(f'<label><input type="checkbox" value="{esc(value)}"><span>{esc(display)}</span></label>' for value, display in values)
        plural = label.casefold() if label.endswith("s") else label.casefold() + "s"
        return f'<details class="multi-filter" data-gh-multi="{field}" data-filter-label="{esc(label)}" data-filter-plural="{esc(plural)}"><summary><span>{esc(label)}</span><strong data-multi-label>All {esc(plural)}</strong></summary><div class="multi-menu"><div class="multi-actions"><button type="button" data-multi-all>Select all</button><button type="button" data-multi-none>Select none</button></div>{choices}</div></details>'
    ...
    compact = [[
        ...
        row.get("first_commit_at"), row.get("coding_hours"),
        people_by_login.get((row.get("author_login") or "").casefold()),
        row.get("coding_time_basis"), row.get("coding_jira_key"),
    ] for row in records]
    payload = json.dumps(compact, separators=(",", ":")).replace("</", "<\\/")
    notes = "".join(f"<li>{esc(note)}</li>" for note in view.get("data_quality_notes", []))
    return f'''<section class="finder-filter-card github-filter-card">
    <div class="finder-search"><span aria-hidden="true">⌕</span><input type="search" data-gh-filter="text" aria-label="Search GitHub records" placeholder="Search title, SHA, author, Jira…"><button type="button" data-gh-search>Search</button></div>
    <div class="finder-filters github-filters">
      {multi_filter("Record", "type", [("pull_request", "Pull requests"), ("commit", "Commits")])}
      {multi_filter("Repository", "repository", pairs(repositories))}
      {multi_filter("State", "state", pairs(states))}
      {multi_filter("Author", "author", pairs(authors))}
      {multi_filter("Team", "team", pairs(teams))}
      {multi_filter("Team mapping", "teamMapping", [("mapped", "Mapped to a Jira team"), ("unmapped", "Not mapped to a Jira team")])}
      {multi_filter("Reviewer", "reviewer", pairs(reviewers))}
      {multi_filter("Jira link", "jira", [("linked", "Linked"), ("unlinked", "Unlinked")])}
      {multi_filter("PR timing data", "timing", [("complete", "Pickup and review present"), ("missing_pickup", "Missing pickup"), ("missing_review", "Missing review"), ("missing_both", "Missing both")])}
      <label>From <input type="date" data-gh-filter="from"></label><label>To <input type="date" data-gh-filter="to"></label>
    </div><div class="github-outlier-controls"><label class="outlier-control github-outlier-control"><input type="checkbox" data-gh-outlier-toggle> Exclude values above thresholds</label><label>Coding <input type="number" min="0" step="1" value="9" data-gh-outlier-threshold="coding"> hours</label><label>Pickup <input type="number" min="0" step="1" value="176" data-gh-outlier-threshold="pickup"> hours</label><label>Review <input type="number" min="0" step="1" value="80" data-gh-outlier-threshold="review"> hours</label></div><div class="outlier-note" data-gh-outlier-note></div><details class="outlier-details" data-gh-outlier-details><summary>Pull requests above the configured thresholds</summary><div class="table-wrap"><table><thead><tr><th>Pull request</th><th>Merged</th><th>Metric</th><th>Value</th><th>Graph status</th></tr></thead><tbody data-gh-outlier-rows></tbody></table></div></details><details class="metric-definitions"><summary>How GitHub timing metrics are computed</summary><ul><li><strong>Coding:</strong> earliest committed timestamp among linked commits that explicitly reference the PR's primary Jira key → PR creation. The primary key is the first configured Jira key found in the PR title, branch name, then body; when no matching commit can be established, this falls back to the earliest linked commit.</li><li><strong>Pickup:</strong> PR creation → first submitted review by someone other than the PR author or a bot.</li><li><strong>Review:</strong> that first qualifying review → merge.</li></ul><p class="table-note">Pickup and review are available only for merged, non-draft PRs with a qualifying review. Negative coding intervals caused by source timestamps are clamped to zero.</p></details><div class="active-filter-row"><span class="active-filter-label" data-gh-active-label>Active filters:</span>
      <div class="active-filter-chips" data-gh-filter-chips></div><button class="filter-clear" type="button" data-gh-clear>Clear all</button>
      <span class="finder-count" data-gh-count aria-live="polite"></span></div></section>
    <div class="finder-secondary-actions"><details class="column-manager github-column-manager"><summary>Columns</summary>
    ...
    <div class="finder-chart-grid">
      <figure class="weekly-chart finder-weekly-chart" data-github-finder-chart="coding"><figcaption><div><strong>PR coding time</strong><span>First primary-ticket commit to PR creation, with an explicit earliest-linked-commit fallback, grouped by merge week.</span></div><div class="chart-kpis"><span>Current average<strong data-gh-chart-average>—</strong></span><span>Trend<strong data-gh-chart-trend>—</strong></span></div></figcaption><div class="weekly-chart-canvas" role="img" aria-label="GitHub Finder weekly average PR coding time chart"></div></figure>
      <figure class="weekly-chart finder-weekly-chart" data-github-finder-chart="pickup"><figcaption><div><strong>PR pickup time</strong><span>PR creation to first qualifying review, grouped by merge week.</span></div><div class="chart-kpis"><span>Current average<strong data-gh-chart-average>—</strong></span><span>Trend<strong data-gh-chart-trend>—</strong></span></div></figcaption><div class="weekly-chart-canvas" role="img" aria-label="GitHub Finder weekly average PR pickup time chart"></div></figure>
      <figure class="weekly-chart finder-weekly-chart" data-github-finder-chart="review"><figcaption><div><strong>PR review time</strong><span>First qualifying review to merge, grouped by merge week.</span></div><div class="chart-kpis"><span>Current average<strong data-gh-chart-average>—</strong></span><span>Trend<strong data-gh-chart-trend>—</strong></span></div></figcaption><div class="weekly-chart-canvas" role="img" aria-label="GitHub Finder weekly average PR review time chart"></div></figure>
    </div>
    <p class="table-note github-coverage">{len(records):,} unique records pinned across {len(repositories):,} configured repositories. Results are paged at 100 rows for local-file performance.</p>
...
def github_pr_metrics_section(view: dict | None) -> str:
    ...
    def metric_group(metric: str, heading: str, explanation: str) -> str:
        ...
        contributions = view.get("contributions", [])
        chart_records = [
            [f'{item["repository"]}#{item["number"]}', item.get("merged_at"), item[field]]
            for item in contributions
        ]
        chart_payload = json.dumps(chart_records, separators=(",", ":")).replace("</", "<\\/")
        rows = "".join(
            f'''<tbody{rag_anchor_attr(item.get(rag_field))} class="pr-metric-contribution rag-instance" data-pr-key="{esc(f'{item["repository"]}#{item["number"]}')}"><tr>
        ...
        <figure class="weekly-chart" data-weekly-chart><figcaption><strong>Weekly {esc(heading.casefold())}</strong><span>Grouped by the UTC Monday of each merge. Point labels show sample size.</span></figcaption><div class="weekly-chart-canvas" role="img" aria-label="Weekly {esc(heading.casefold())} chart"></div></figure>
        <p class="table-note">{esc(explanation)} Top five contributors for the selected merge-date range.</p>
        <div class="table-wrap"><table class="pr-metric-table"><thead><tr><th>Pull request</th><th class="num">Time</th><th>Created → first review → merged</th><th>Involved people</th><th>Title</th></tr></thead>{rows}</table></div>
        <script type="application/json" class="pr-metric-records">{chart_payload}</script></section>'''
...
.toggle{font:inherit;font-size:12px;color:#315bd6;background:#eef2fc;border:1px solid #d3ddf4;border-radius:20px;padding:4px 10px;cursor:pointer}.toggle:hover{background:#e2e9f9}.toggle[aria-expanded="true"]{background:#dbe4f8}.cycle-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin:10px 0}.cycle-summary .metric{padding:12px}.cycle-summary .metric strong{font-size:22px}.cycle-group{margin:22px 0}.cycle-child td{background:#fafbfe;font-size:12px}.top-status{background:#e7ddff;color:#57359a}.cycle-excluded{display:none}.cycle-excluded.focused{display:table-row-group}.weekly-chart{margin:14px 0;padding:14px;border:1px solid #dfe4ef;border-radius:10px;background:#fbfcff}.weekly-chart figcaption{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap}.weekly-chart figcaption span{color:#667087;font-size:12px}.outlier-control{display:inline-flex;align-items:center;gap:7px;margin-top:10px;font-size:13px;font-weight:600}.outlier-note{margin-top:6px;color:#667087;font-size:12px}.weekly-chart-canvas{min-height:230px;margin-top:8px}.weekly-chart svg{display:block;width:100%;height:auto;overflow:visible}.chart-grid{stroke:#dfe4ef;stroke-width:1}.chart-axis{fill:#667087;font-size:11px}.chart-line{fill:none;stroke:#315bd6;stroke-width:3;stroke-linejoin:round;stroke-linecap:round}.chart-point{fill:#fff;stroke:#315bd6;stroke-width:3}.chart-label{fill:#26355c;font-size:11px;font-weight:600}.chart-empty{display:grid;place-items:center;min-height:210px;color:#667087;font-style:italic}.rag-badge{display:inline-flex;align-items:center;gap:4px;padding:3px 7px;border-radius:5px;font-size:12px;font-weight:700;white-space:nowrap}.rag-red{background:#ffe3e1;color:#982d28;border:1px solid #e9aaa5}.rag-amber{background:#fff0c4;color:#745100;border:1px solid #e5c66c}.rag-green{background:#dff6e8;color:#17633b;border:1px solid #96d5af}.rag-summary ul{columns:2;padding-left:22px}.rag-summary li{break-inside:avoid;margin:6px 0}.rag-instance:target,.rag-instance.focused{outline:3px solid #315bd6;outline-offset:-2px;scroll-margin-top:90px}.landing-header{margin:20px 0 28px}.landing-header h1{margin:5px 0}.landing-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}.landing-section{position:relative;background:white;border:1px solid #dfe4ef;border-radius:14px;padding:24px;box-shadow:0 3px 12px #26334d10;min-height:190px}.landing-section h2{font-size:24px;margin:4px 0}.landing-section p{color:#667087;margin:0 0 18px}.landing-number{color:#8792aa;font-size:12px;font-weight:700;letter-spacing:.1em}.directory-links{display:flex;flex-wrap:wrap;gap:8px}.directory-link,.finder-link{display:inline-flex;align-items:center;padding:8px 11px;border-radius:8px;background:#edf1fb;text-decoration:none;font-weight:600}.directory-link:hover,.finder-link:hover{background:#dfe7fa}.finder-link{margin-top:8px}.finder-stub{max-width:720px;margin:60px auto;text-align:center;padding:50px}.finder-filters{display:flex;align-items:end;gap:10px;flex-wrap:wrap;margin:14px 0}.finder-filters label{display:grid;gap:4px;color:#667087;font-size:12px}.finder-filters select{font:inherit;min-width:170px;padding:7px 9px;border:1px solid #bcc6dc;border-radius:7px;background:white}.finder-count{color:#52596b;font-size:13px;margin-left:auto}.facet-hidden{display:none!important}.column-manager,.threshold-manager{background:#f8faff;border:1px solid #dfe4ef;border-radius:10px;padding:10px 12px;margin:12px 0}.column-manager>summary,.threshold-manager>summary{color:#315bd6}.column-manager-visible{display:flex;flex-wrap:wrap;gap:7px;margin:10px 0}.column-item{display:inline-flex;align-items:center;gap:3px;background:white;border:1px solid #dfe4ef;border-radius:8px;padding:4px 5px 4px 9px}.column-item strong{font-size:12px}.column-item button{border:0;background:#edf1fb;color:#315bd6;border-radius:5px;cursor:pointer;padding:2px 6px}.column-item button:disabled{color:#9ba4b7;cursor:not-allowed}.column-add{display:flex;align-items:end;gap:7px}.column-add label{display:grid;gap:3px;color:#667087;font-size:12px}.column-add select{font:inherit;padding:6px 8px;border:1px solid #bcc6dc;border-radius:7px;background:white}.column-hidden{display:none!important}.threshold-grid{display:grid;grid-template-columns:minmax(180px,1fr) 130px 130px;gap:7px;align-items:center;max-width:520px;margin:10px 0}.threshold-grid input{font:inherit;padding:6px 8px;border:1px solid #bcc6dc;border-radius:7px;min-width:0}.threshold-actions{display:flex;align-items:center;gap:10px}.cell-red{background:#ffe3e1!important;color:#982d28;font-weight:700}.cell-amber{background:#fff0c4!important;color:#745100;font-weight:700}.cell-red::before{content:"● ";color:#b42318}.cell-amber::before{content:"▲ ";color:#9a6700}.quality-missing{background:repeating-linear-gradient(135deg,#f2f4f8,#f2f4f8 5px,#e5e9f1 5px,#e5e9f1 10px)!important;color:#52596b}.quality-missing::before{content:"⚠ ";color:#596579}.attention-hidden{display:none!important}@media(max-width:650px){nav{padding:8px 16px}.generated{max-width:150px}.card header,.team-card header{display:block}.team-card{padding:15px}.landing-grid{grid-template-columns:1fr}.landing-section{min-height:0}th,td{min-width:110px}th:last-child,td:last-child{min-width:220px}.rag-summary ul{columns:1}.finder-count{width:100%;margin-left:0}.threshold-grid{grid-template-columns:minmax(130px,1fr) 90px 90px}}'''
CSS += '''
main{max-width:1440px;padding:32px 32px 52px}nav{padding:8px max(32px,calc((100vw - 1440px)/2 + 32px));justify-content:flex-start}nav .brand img{width:34px;height:34px}.nav-links{display:flex;align-self:stretch;margin-left:34px}.nav-links a{display:flex;align-items:center;padding:0 18px;color:#d9e2f3;text-decoration:none;border-bottom:3px solid transparent}.nav-links a:hover{color:white;background:#ffffff0a}.generated{margin-left:auto}
.finder-heading{display:flex;justify-content:space-between;align-items:end;margin-bottom:16px}.finder-heading h1{margin:0}.finder-heading p{margin:2px 0 0}.finder-heading-count{font-size:22px;font-weight:750;color:#172033}
.finder-filter-card{background:white;border:1px solid #dfe4ef;border-radius:12px;padding:14px 16px 10px;box-shadow:0 2px 8px #26334d0b}.finder-search{height:42px;display:flex;align-items:center;gap:9px;border:1px solid #cbd4e5;border-radius:8px;padding:0 5px 0 12px}.finder-search:focus-within{border-color:#315bd6;box-shadow:0 0 0 3px #315bd619}.finder-search span{font-size:22px;color:#667087}.finder-search input{width:100%;border:0;outline:0;background:transparent;font:inherit}.finder-search button{height:32px;padding:0 15px;border:0;border-radius:6px;background:#315bd6;color:white;font:inherit;font-weight:700;cursor:pointer}.finder-search button:hover{background:#2549b7}.finder-filters{display:grid;grid-template-columns:repeat(6,minmax(140px,1fr));align-items:end;gap:12px;margin:14px 0 10px}.finder-filters label{display:grid;gap:5px;color:#667087;font-size:12px}.finder-filters select,.finder-filters input,.multi-filter summary{box-sizing:border-box;width:100%;min-width:0;height:38px;padding:8px 10px;border:1px solid #cbd4e5;border-radius:7px;background:white;font:inherit;color:#172033}.multi-filter{position:relative;margin:0}.multi-filter summary{display:grid;gap:1px;height:auto;min-height:38px;cursor:pointer;list-style:none;padding:5px 28px 5px 10px;position:relative}.multi-filter summary::-webkit-details-marker{display:none}.multi-filter summary::after{content:'⌄';position:absolute;right:10px;top:10px}.multi-filter summary span{font-size:10px;color:#667087}.multi-filter summary strong{font-size:13px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.multi-menu{position:absolute;z-index:8;top:calc(100% + 5px);left:0;width:max-content;min-width:220px;max-height:280px;overflow:auto;padding:8px;background:white;border:1px solid #cbd4e5;border-radius:9px;box-shadow:0 12px 30px #17203324}.multi-menu label{display:flex;align-items:center;gap:8px;padding:7px 8px;border-radius:6px;color:#26355c;font-size:13px}.multi-menu label:hover{background:#f0f4fc}.multi-menu input{width:auto;height:auto;min-width:0}.active-filter-row{min-height:32px;border-top:1px solid #edf0f6;padding-top:9px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12px}.active-filter-label{color:#667087}.active-filter-chips{display:flex;gap:6px;flex-wrap:wrap}.filter-chip{border:1px solid #d5deef;background:#f4f7fd;color:#405071;border-radius:16px;padding:4px 9px}.filter-clear{border:0;background:transparent;color:#315bd6;font-weight:700;cursor:pointer;padding:5px 7px}.finder-count{margin-left:auto;font-weight:650}.date-note{color:#745100}
.finder-secondary-actions{display:flex;justify-content:flex-end;gap:8px;margin:10px 0}.finder-secondary-actions details{position:relative;margin:0}.finder-secondary-actions details>summary{list-style:none;color:#315bd6;background:white;border:1px solid #d7deec;border-radius:7px;padding:7px 11px;font-size:12px}.finder-secondary-actions details>summary::-webkit-details-marker{display:none}.finder-secondary-actions details[open]{padding:0;border:0;background:transparent}.finder-secondary-actions details[open]>:not(summary){position:relative;z-index:4;background:white}.finder-secondary-actions .threshold-manager[open]{width:min(620px,100%)}.finder-secondary-actions .column-manager[open]{width:min(620px,100%)}
.finder-weekly-chart{margin:0 0 14px;padding:16px 18px;border-radius:12px;background:white;box-shadow:0 2px 8px #26334d0b}.finder-weekly-chart figcaption{align-items:flex-start}.finder-weekly-chart figcaption>div:first-child{max-width:700px}.finder-weekly-chart figcaption strong{display:block;font-size:18px}.finder-weekly-chart figcaption span{display:block}.chart-kpis{display:flex;gap:0;margin-left:auto}.chart-kpis>span{min-width:110px;padding:0 16px;border-left:1px solid #dfe4ef;font-size:11px;color:#667087}.chart-kpis strong{font-size:20px;color:#172033;margin-top:2px}.finder-weekly-chart .weekly-chart-canvas{height:300px;min-height:300px}.outlier-control{margin-top:4px}.outlier-note{display:inline;margin-left:10px}.outlier-details{border-top:1px solid #e3e8f1;padding-top:9px}.outlier-details .table-wrap{margin-top:8px;max-height:260px;overflow:auto}
.issue-finder-table{font-size:12px}.issue-finder-table th{background:#f7f9fd}.issue-finder-table tbody tr:hover td{background:#f7faff}.issue-finder-table td,.issue-finder-table th{padding:8px 10px}
.github-filter-card{margin-bottom:10px}.github-filter-card .github-filters{grid-template-columns:repeat(6,minmax(135px,1fr))}.github-finder-table{font-size:12px}.github-finder-table th{background:#f7f9fd}.github-finder-table tbody tr:hover td{background:#f7faff}.github-finder-table td,.github-finder-table th{padding:8px 10px}.github-coverage{margin:2px 0 8px}.github-pager{display:flex;justify-content:center;align-items:center;gap:10px;padding:12px}.finder-chart-grid .finder-weekly-chart{margin:0}.finder-chart-grid .weekly-chart-canvas{height:270px;min-height:270px}
.multi-actions{display:flex;gap:6px;padding:2px 4px 8px;margin-bottom:4px;border-bottom:1px solid #e7ebf3}.multi-actions button{border:0;background:#eef3fc;color:#315bd6;border-radius:6px;padding:5px 8px;font:inherit;font-size:12px;font-weight:650;cursor:pointer}.multi-actions button:hover{background:#dfe8fa}
.github-filter-card .github-filters{grid-template-columns:repeat(auto-fit,minmax(180px,1fr))}.github-outlier-control{margin-top:14px}
.github-outlier-controls{display:flex;align-items:end;gap:14px;flex-wrap:wrap;margin-top:12px}.github-outlier-controls>label:not(.outlier-control){display:flex;align-items:center;gap:5px;color:#667087;font-size:12px}.github-outlier-controls input[type="number"]{width:82px;padding:6px 8px;border:1px solid #cbd4e5;border-radius:7px;font:inherit}.metric-definitions{margin:10px 0;color:#405071}.metric-definitions li{margin:5px 0}
@media(max-width:950px){.finder-filters{grid-template-columns:repeat(3,1fr)}.nav-links{display:none}.chart-kpis{width:100%;margin-top:10px}.finder-weekly-chart .weekly-chart-canvas{height:260px}}
'''
...
document.querySelectorAll('.table-wrap').forEach(w=>{const t=w.querySelector('table');if(!t||!t.tHead||!t.tBodies.length||w.closest('.outlier-details')||t.classList.contains('issue-finder-table')||t.classList.contains('cycle-table')||t.classList.contains('pr-metric-table')||t.querySelector('.child-row,.month-row'))return;
...
function dateControls(label='Table dates'){const box=document.createElement('div');box.className='date-controls';box.innerHTML='<strong>'+label+'</strong><label>From <input type="date" data-date-from></label><label>To <input type="date" data-date-to></label><button class="toggle" type="button" data-date-clear>Clear</button><span class="date-note"></span>';return box;}
function bindDateScope(scope,targets,dateValue,after,label){const controls=dateControls(label),anchor=scope.querySelector('.table-wrap')||scope;anchor.parentNode.insertBefore(controls,anchor);const from=controls.querySelector('[data-date-from]'),to=controls.querySelector('[data-date-to]'),note=controls.querySelector('.date-note');function applyDates(){const first=from.value,last=to.value,active=Boolean(first||last);let undated=0;const included=[];targets().forEach(row=>{const d=dateValue(row);let hide=false;if(active){if(!d){hide=true;undated++;}else hide=Boolean((first&&d<first)||(last&&d>last));}const element=row.element||row;if(element.classList)element.classList.toggle('date-hidden',hide);if(!hide)included.push(row);});if(after)after(included,targets());note.textContent=active?(undated?undated+' rows without a Done date hidden':'Filter active'):'';}from.addEventListener('change',applyDates);to.addEventListener('change',applyDates);controls.querySelector('[data-date-clear]').addEventListener('click',()=>{from.value='';to.value='';applyDates();});applyDates();}
function quantile(values,p){if(!values.length)return null;const index=(values.length-1)*p,lower=Math.floor(index),fraction=index-lower;return values[lower]+((values[lower+1]??values[lower])-values[lower])*fraction;}
function extremeHighOutliers(records,valueOf){if(records.length<4)return {outliers:[],fence:null};const values=records.map(valueOf).filter(Number.isFinite).sort((a,b)=>a-b),q1=quantile(values,.25),q3=quantile(values,.75),fence=q3+3*(q3-q1);return {outliers:records.filter(record=>valueOf(record)>fence),fence};}
...
function formatHours(value){return value.toFixed(1)+' h';}
function renderWeeklyAverageChart(container,records,options){const points=weeklyAverage(records,options.date,options.value),unit=options.unit||'units',shortUnit=options.shortUnit||unit,digits=options.digits??1,recordLabel=options.recordLabel||'record';container.setAttribute('aria-label',options.label+': '+points.map(p=>'week of '+p.week+', '+p.average.toFixed(2)+' '+unit+', '+p.count+' '+recordLabel+(p.count===1?'':'s')).join('; '));if(!points.length){container.innerHTML='<div class="chart-empty">No qualifying records in the selected range.</div>';return;}const width=900,height=250,left=58,right=22,top=26,bottom=48,plotW=width-left-right,plotH=height-top-bottom,max=Math.max(...points.map(p=>p.average),1),x=i=>left+(points.length===1?plotW/2:i*plotW/(points.length-1)),y=v=>top+plotH-v/max*plotH,detail=p=>'Week of '+p.week+': '+p.average.toFixed(2)+' '+unit+' across '+p.count+' '+recordLabel+(p.count===1?'':'s');const ticks=[0,.25,.5,.75,1].map(f=>{const yy=y(max*f);return '<line class="chart-grid" x1="'+left+'" x2="'+(width-right)+'" y1="'+yy+'" y2="'+yy+'"/><text class="chart-axis" x="'+(left-8)+'" y="'+(yy+4)+'" text-anchor="end">'+(max*f).toFixed(1)+'</text>'}).join(''),path=points.map((p,i)=>(i?'L':'M')+x(i)+' '+y(p.average)).join(' '),marks=points.map((p,i)=>'<g><circle class="chart-point" '+(options.interactiveDetails?'tabindex="0" role="button" data-point="'+i+'" ':'')+'cx="'+x(i)+'" cy="'+y(p.average)+'" r="5"><title>'+detail(p)+'</title></circle>'+(options.interactiveDetails?'':'<text class="chart-label" x="'+x(i)+'" y="'+(y(p.average)-11)+'" text-anchor="middle">'+p.average.toFixed(digits)+shortUnit+' · n='+p.count+'</text>')+'<text class="chart-axis" x="'+x(i)+'" y="'+(height-19)+'" text-anchor="middle">'+p.week.slice(5)+'</text></g>').join('');container.innerHTML='<svg viewBox="0 0 '+width+' '+height+'" aria-hidden="'+(options.interactiveDetails?'false':'true')+'">'+ticks+'<text class="chart-axis" transform="translate(15 '+(top+plotH/2)+') rotate(-90)" text-anchor="middle">'+options.axisLabel+'</text><path class="chart-line" d="'+path+'"/>'+marks+'</svg>'+(options.interactiveDetails?'<div class="chart-detail" aria-live="polite">Hover, focus, or select a point for details.</div>':'');if(options.interactiveDetails){const output=container.querySelector('.chart-detail');container.querySelectorAll('[data-point]').forEach(node=>{const show=()=>output.textContent=detail(points[Number(node.dataset.point)]);node.addEventListener('mouseenter',show);node.addEventListener('focus',show);node.addEventListener('click',show);});}}
function renderIssueFinderChart(container,records){const points=weeklyAverage(records,record=>record.ended,record=>record.days),scope=container.closest('[data-issue-finder-chart]'),average=points.length?points.at(-1).average:null,first=points.length?points[0].average:null,trend=average!==null&&first?((average-first)/first)*100:null;scope.querySelector('[data-chart-average]').textContent=average===null?'—':average.toFixed(1)+'d';scope.querySelector('[data-chart-sample]').textContent=String(records.length);scope.querySelector('[data-chart-trend]').textContent=trend===null?'—':(trend>0?'+':'')+trend.toFixed(0)+'%';if(!container._chart){container._chart=echarts.init(container,null,{renderer:'svg'});window.addEventListener('resize',()=>container._chart.resize());}container._chart.setOption({animationDuration:350,grid:{left:54,right:18,top:28,bottom:34},tooltip:{trigger:'item',triggerOn:'mousemove|click',confine:true,backgroundColor:'#172033',borderWidth:0,padding:[10,12],textStyle:{color:'#fff'},formatter:param=>'<strong>Week of '+param.name+'</strong><br><span style="font-size:18px;font-weight:700">'+param.data.value.toFixed(2)+' days</span><br>'+param.data.count+' completed issue'+(param.data.count===1?'':'s')},xAxis:{type:'category',boundaryGap:false,data:points.map(point=>point.week),axisLine:{lineStyle:{color:'#cfd7e6'}},axisTick:{show:false},axisLabel:{color:'#667087',formatter:value=>value.slice(5)}},yAxis:{type:'value',name:'Average calendar days',nameLocation:'middle',nameGap:42,nameTextStyle:{color:'#667087'},axisLabel:{color:'#667087'},splitLine:{lineStyle:{color:'#e4e9f2',type:'dashed'}}},series:[{type:'line',smooth:.25,symbol:'circle',symbolSize:9,showSymbol:true,lineStyle:{color:'#2f6fed',width:3},itemStyle:{color:'#fff',borderColor:'#2f6fed',borderWidth:3},emphasis:{scale:1.5},areaStyle:{color:new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'rgba(47,111,237,.20)'},{offset:1,color:'rgba(47,111,237,.02)'}])},data:points.map(point=>({value:point.average,count:point.count}))}]},true);container.setAttribute('aria-label','Issue Finder weekly average completed Build Cycle Time: '+points.map(point=>'week of '+point.week+', '+point.average.toFixed(2)+' days, '+point.count+' completed issues').join('; '));}
function renderGithubFinderChart(container,records,dateOf,valueOf,label){const points=weeklyAverage(records,dateOf,valueOf),figure=container.closest('[data-github-finder-chart]'),current=points.at(-1)?.average,first=points[0]?.average,delta=current!==undefined&&first!==undefined?current-first:null;figure.querySelector('[data-gh-chart-average]').textContent=current===undefined?'—':formatHours(current);figure.querySelector('[data-gh-chart-trend]').textContent=delta===null||points.length<2?'—':(delta>0?'▲ ':'▼ ')+formatHours(Math.abs(delta));if(!container._chart){container._chart=echarts.init(container,null,{renderer:'svg'});window.addEventListener('resize',()=>container._chart.resize());}container._chart.setOption({animationDuration:350,grid:{left:50,right:16,top:22,bottom:34},tooltip:{trigger:'item',triggerOn:'mousemove|click',confine:true,backgroundColor:'#172033',borderWidth:0,padding:[10,12],textStyle:{color:'#fff'},formatter:param=>'<strong>Week of '+param.name+'</strong><br><span style="font-size:18px;font-weight:700">'+param.data.value.toFixed(2)+' hours</span><br>'+param.data.count+' pull request'+(param.data.count===1?'':'s')},xAxis:{type:'category',boundaryGap:false,data:points.map(point=>point.week),axisLine:{lineStyle:{color:'#cfd7e6'}},axisTick:{show:false},axisLabel:{color:'#667087',formatter:value=>value.slice(5)}},yAxis:{type:'value',axisLabel:{color:'#667087',formatter:value=>value+'h'},splitLine:{lineStyle:{color:'#e4e9f2',type:'dashed'}}},series:[{type:'line',smooth:.25,symbol:'circle',symbolSize:8,showSymbol:true,lineStyle:{color:'#2f6fed',width:3},itemStyle:{color:'#fff',borderColor:'#2f6fed',borderWidth:3},emphasis:{scale:1.5},areaStyle:{color:new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'rgba(47,111,237,.18)'},{offset:1,color:'rgba(47,111,237,.02)'}])},data:points.map(point=>({value:point.average,count:point.count}))}]},true);container.setAttribute('aria-label',label+': '+points.map(point=>'week of '+point.week+', '+point.average.toFixed(2)+' hours, '+point.count+' pull requests').join('; '));}
document.querySelectorAll('.cycle-group').forEach(group=>{const raw=JSON.parse(group.querySelector('.cycle-records').textContent),records=raw.map(item=>({key:item[0],ended:item[1],days:item[2],statuses:item[3],element:group.querySelector('[data-cycle-key="'+CSS.escape(item[0])+'"]')}));bindDateScope(group,()=>records,record=>record.ended,(included,all)=>{const ranked=[...included].sort((a,b)=>b.days-a.days);all.forEach(record=>record.element.classList.toggle('cycle-excluded',!ranked.slice(0,5).includes(record)));const total=included.reduce((sum,record)=>sum+record.days,0),statuses={};included.forEach(record=>Object.entries(record.statuses).forEach(([status,days])=>statuses[status]=(statuses[status]||0)+Number(days)));const top=Object.entries(statuses).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]))[0];group.querySelector('.cycle-average').textContent=included.length?(total/included.length).toFixed(2):'—';group.querySelector('.cycle-sample').textContent=String(included.length);group.querySelector('.cycle-top-status').textContent=top?top[0]+' ('+top[1].toFixed(2)+'d)':'—';renderWeeklyAverageChart(group.querySelector('.weekly-chart-canvas'),included,{date:record=>record.ended,value:record=>record.days,label:'Weekly average cycle time',unit:'calendar days',shortUnit:'d',axisLabel:'Average calendar days',recordLabel:'issue'});});});
...
document.querySelectorAll('.table-wrap').forEach(w=>{if(w.closest('.cycle-group,.pr-metric-group')||w.querySelector('.issue-finder-table'))return;const dated=[...w.querySelectorAll('[data-date]')];if(dated.length)bindDateScope(w,()=>dated,row=>row.dataset.date);});
document.querySelectorAll('.issue-finder-table').forEach(table=>{const scope=table.closest('.app-view'),rows=[...table.tBodies[0].rows].filter(r=>r.cells.length>1),controls=[...scope.querySelectorAll('[data-finder-field]')],multis=[...scope.querySelectorAll('[data-finder-multi]')],attention=scope.querySelector('[data-finder-attention]'),text=scope.querySelector('[data-issue-finder-text]'),from=scope.querySelector('[data-issue-date-from]'),to=scope.querySelector('[data-issue-date-to]'),dateNote=scope.querySelector('[data-issue-date-note]'),count=scope.querySelector('.finder-count'),chips=scope.querySelector('[data-active-filter-chips]');function multiValues(control){return new Set([...control.querySelectorAll('input:checked')].map(input=>input.value));}function renderFilters(){const labels=[];multis.forEach(control=>{const values=[...multiValues(control)],name=control.dataset.finderMulti==='issueTeam'?'Team':'Status',output=control.querySelector('[data-multi-label]');output.textContent=values.length?values.length+' selected':'All '+name.toLowerCase()+'s';if(values.length)labels.push(name+': '+values.length+' selected');});if(from.value)labels.push('Done from: '+from.value);if(to.value)labels.push('Done to: '+to.value);if(controls[0]?.value)labels.push('Classification: '+controls[0].selectedOptions[0].text);if(attention.value)labels.push('Highlight: '+attention.selectedOptions[0].text);chips.replaceChildren(...labels.map(label=>{const chip=document.createElement('span');chip.className='filter-chip';chip.textContent=label;return chip;}));scope.querySelector('.active-filter-label').hidden=!labels.length;}function applyFacets(){const query=text.value.trim().toLowerCase(),first=from.value,last=to.value,dateActive=Boolean(first||last),multiSelections=new Map(multis.map(control=>[control.dataset.finderMulti,multiValues(control)]));let undated=0;rows.forEach(row=>{const selectHide=controls.some(control=>control.value&&row.dataset[control.dataset.finderField]!==control.value),multiHide=[...multiSelections].some(([field,values])=>values.size&&!values.has(row.dataset[field])),value=attention.value,level=row.dataset.attention||'none',attentionHide=Boolean(value)&&!((value==='flagged'&&(level==='red'||level==='amber'))||level===value),done=row.dataset.cycleEnded,dateHide=dateActive&&(!done||Boolean((first&&done<first)||(last&&done>last)));if(dateActive&&!done)undated++;row.classList.toggle('facet-hidden',selectHide||multiHide);row.classList.toggle('attention-hidden',attentionHide);row.classList.toggle('date-hidden',dateHide);row.classList.toggle('text-hidden',Boolean(query)&&!row.innerText.toLowerCase().includes(query));});const shown=rows.filter(row=>!['facet-hidden','attention-hidden','date-hidden','text-hidden'].some(name=>row.classList.contains(name))).length;count.textContent=shown+' of '+rows.length+' issues';dateNote.textContent=dateActive&&undated?undated+' without a Done date hidden':'';renderFilters();}controls.forEach(control=>control.addEventListener('change',applyFacets));multis.forEach(control=>control.addEventListener('change',applyFacets));attention.addEventListener('change',applyFacets);text.addEventListener('input',applyFacets);from.addEventListener('change',applyFacets);to.addEventListener('change',applyFacets);scope.addEventListener('finder-attention-change',applyFacets);scope.querySelector('[data-finder-clear]').addEventListener('click',()=>{controls.forEach(control=>control.value='');multis.forEach(control=>control.querySelectorAll('input').forEach(input=>input.checked=false));attention.value='';text.value='';from.value='';to.value='';applyFacets();});applyFacets();});
function syncIssueMultiLabels(scope){const controls=[...scope.querySelectorAll('[data-finder-multi]')],chips=[...scope.querySelectorAll('[data-active-filter-chips] .filter-chip')];let chipIndex=0;controls.forEach(control=>{const values=[...control.querySelectorAll('input:checked')].map(input=>input.value),total=control.querySelectorAll('input[type="checkbox"]').length,label=control.dataset.filterLabel,plural=control.dataset.filterPlural,selection=values.length===total?'All '+plural:values.join(', '),output=control.querySelector('[data-multi-label]');output.textContent=values.length?selection:'All '+plural;output.title=values.length?selection:'';if(values.length&&chips[chipIndex]){chips[chipIndex].textContent=label+': '+selection;chips[chipIndex++].title=selection;}});}
function deferFinderSearch(scope,inputSelector,buttonSelector,clearSelector){const bound=scope.querySelector(inputSelector),input=bound.cloneNode(true),button=scope.querySelector(buttonSelector);bound.replaceWith(input);function apply(){bound.value=input.value;bound.dispatchEvent(new Event('input',{bubbles:true}));}button.addEventListener('click',apply);input.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();apply();}});scope.querySelector(clearSelector).addEventListener('click',()=>{input.value='';});}
document.querySelectorAll('.issue-finder-view').forEach(scope=>{scope.querySelectorAll('[data-finder-multi]').forEach(control=>{control.addEventListener('change',()=>requestAnimationFrame(()=>syncIssueMultiLabels(scope)));control.querySelector('[data-multi-all]').addEventListener('click',()=>{control.querySelectorAll('input').forEach(input=>input.checked=true);control.dispatchEvent(new Event('change',{bubbles:true}));});control.querySelector('[data-multi-none]').addEventListener('click',()=>{control.querySelectorAll('input').forEach(input=>input.checked=false);control.dispatchEvent(new Event('change',{bubbles:true}));});});scope.querySelectorAll('[data-finder-field],[data-finder-attention],[data-issue-date-from],[data-issue-date-to]').forEach(control=>control.addEventListener('change',()=>requestAnimationFrame(()=>syncIssueMultiLabels(scope))));scope.querySelector('[data-finder-clear]').addEventListener('click',()=>requestAnimationFrame(()=>syncIssueMultiLabels(scope)));deferFinderSearch(scope,'[data-issue-finder-text]','[data-issue-search]','[data-finder-clear]');scope.querySelector('[data-issue-search]').addEventListener('click',()=>requestAnimationFrame(()=>syncIssueMultiLabels(scope)));syncIssueMultiLabels(scope);});
document.querySelectorAll('.issue-finder-table').forEach(table=>{const scope=table.closest('.app-view'),chart=scope.querySelector('[data-issue-finder-chart]'),canvas=chart.querySelector('.weekly-chart-canvas'),toggle=chart.querySelector('[data-issue-outlier-toggle]'),note=chart.querySelector('[data-issue-outlier-note]'),outlierRows=chart.querySelector('[data-issue-outlier-rows]'),rows=[...table.tBodies[0].rows].filter(row=>row.cells.length>1);let queued=false;function update(){queued=false;const records=rows.filter(row=>!['facet-hidden','attention-hidden','date-hidden','text-hidden'].some(name=>row.classList.contains(name))&&row.dataset.cycleEnded&&row.dataset.totalCycleDays!=='').map(row=>({key:row.querySelector('a')?.textContent.trim()||'Unknown',link:row.querySelector('a'),ended:row.dataset.cycleEnded,days:Number(row.dataset.totalCycleDays)})),result=extremeHighOutliers(records,record=>record.days),outliers=new Set(result.outliers),excluded=toggle.checked?outliers:new Set(),plotted=records.filter(record=>!excluded.has(record));if(result.fence===null)note.textContent='Extreme outlier detection requires at least four qualifying Done issues.';else if(!result.outliers.length)note.textContent='No extreme outliers detected. Rule: values above Q3 + 3×IQR ('+result.fence.toFixed(2)+'d).';else note.textContent=(toggle.checked?'Excluded ':'Detected ')+result.outliers.length+' extreme outlier'+(result.outliers.length===1?'':'s')+' above Q3 + 3×IQR ('+result.fence.toFixed(2)+'d).';outlierRows.replaceChildren(...records.map(record=>{const row=document.createElement('tr'),issue=row.insertCell();if(record.link)issue.append(record.link.cloneNode(true));else issue.textContent=record.key;row.insertCell().textContent=record.ended;row.insertCell().textContent=record.days.toFixed(2)+'d';row.insertCell().textContent=excluded.has(record)?'Excluded':outliers.has(record)?'Included (outlier detected)':'Included';return row;}));renderIssueFinderChart(canvas,plotted);}toggle.addEventListener('change',update);new MutationObserver(()=>{if(!queued){queued=true;requestAnimationFrame(update)}}).observe(table,{subtree:true,attributes:true,attributeFilter:['class']});update();});
window.addEventListener('hashchange',()=>requestAnimationFrame(()=>document.querySelectorAll('.weekly-chart-canvas').forEach(canvas=>canvas._chart?.resize())));
...
document.querySelectorAll('.github-finder-table').forEach(table=>{const scope=table.closest('.app-view'),raw=JSON.parse(scope.querySelector('#github-finder-data').textContent),rows=raw.map(r=>({type:r[0],repository:r[1],identifier:r[2],title:r[3],url:r[4],state:r[5],draft:r[6],author:r[7],created:r[8],updated:r[9],merged:r[10],authored:r[11],committed:r[12],head:r[13],base:r[14],commits:r[15],reviews:r[16],reviewers:r[17],prs:r[18],jira:r[19],jiraUrls:r[20],authorTeams:r[21],reviewerTeams:r[22],firstReviewed:r[23],pickupHours:r[24],reviewHours:r[25],firstCommit:r[26],codingHours:r[27],authorPerson:r[28],codingBasis:r[29],codingJira:r[30]}));
const columns=[['type','Record'],['repository','Repository'],['identifier','ID'],['title','Title'],['state','State'],['author','Author'],['authorPerson','Mapped person'],['authorTeams','Mapped team(s)'],['date','Date'],['codingHours','PR coding time'],['codingBasis','Coding basis'],['pickupHours','PR pickup time'],['reviewHours','PR review time'],['head','Head'],['base','Base'],['commits','Commits'],['reviews','Reviews'],['reviewers','Reviewers'],['prs','Pull requests'],['jira','Jira']];let visible=columns.map(c=>c[0]),page=0,sortKey='date',sortDir='desc',filtered=[];const size=100,head=table.tHead.rows[0],body=table.tBodies[0],manager=scope.querySelector('.github-column-manager'),filters=Object.fromEntries([...scope.querySelectorAll('[data-gh-filter]')].map(x=>[x.dataset.ghFilter,x])),multis=[...scope.querySelectorAll('[data-gh-multi]')],outlierToggle=scope.querySelector('[data-gh-outlier-toggle]'),outlierThresholds=Object.fromEntries([...scope.querySelectorAll('[data-gh-outlier-threshold]')].map(input=>[input.dataset.ghOutlierThreshold,input])),outlierNote=scope.querySelector('[data-gh-outlier-note]'),outlierRows=scope.querySelector('[data-gh-outlier-rows]');
const e=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),date=r=>r.merged||r.updated||r.committed||r.authored||r.created||'',hours=v=>v===null||v===undefined?'—':formatHours(Number(v)),label=k=>columns.find(c=>c[0]===k)[1],cell=(r,k)=>{if(k==='identifier'||k==='title')return '<a href="'+e(r.url)+'" target="_blank" rel="noreferrer">'+e(r[k])+'</a>';if(k==='type')return r.type==='pull_request'?'Pull request':'Commit';if(k==='state')return e((r.draft?'draft · ':'')+(r.state||'—'));if(k==='date')return date(r)?'<time datetime="'+e(date(r))+'">'+e(date(r).slice(0,10))+'</time>':'—';if(k==='codingHours'||k==='pickupHours'||k==='reviewHours')return e(hours(r[k]));if(k==='codingBasis')return r.codingBasis==='primary_jira_key'?'Primary Jira key '+e(r.codingJira):r.codingBasis==='fallback_no_matching_jira_commit'?'Fallback: no commit matched '+e(r.codingJira):r.codingBasis==='fallback_no_primary_jira'?'Fallback: no primary Jira key':'—';if(k==='authorTeams'||k==='reviewers'||k==='prs')return e((r[k]||[]).join(', ')||'—');if(k==='jira')return r.jira.length?r.jira.map(j=>'<a href="'+e(r.jiraUrls[j])+'" target="_blank" rel="noreferrer">'+e(j)+'</a>').join(', '):'—';return e(r[k]??'—')};
...
function multiValues(control){return new Set([...control.querySelectorAll('input:checked')].map(input=>input.value))}function multiLabels(control){return [...control.querySelectorAll('input:checked')].map(input=>input.closest('label').innerText.trim())}function selected(field){return multiValues(multis.find(control=>control.dataset.ghMulti===field))}function selectedMatch(values,value){return !values.size||values.has(value)}function selectedAny(values,items){return !values.size||items.some(item=>values.has(item))}
function finderCharts(){const detected=[];[['coding','codingHours'],['pickup','pickupHours'],['review','reviewHours']].forEach(([metric,field])=>{const records=filtered.filter(row=>row.type==='pull_request'&&row.merged&&row[field]!==null&&row[field]!==undefined),threshold=Number(outlierThresholds[metric].value),outliers=Number.isFinite(threshold)?new Set(records.filter(row=>row[field]>threshold)):new Set(),plotted=outlierToggle.checked?records.filter(row=>!outliers.has(row)):records;outliers.forEach(row=>detected.push({row,metric,value:row[field],excluded:outlierToggle.checked}));renderGithubFinderChart(scope.querySelector('[data-github-finder-chart="'+metric+'"] .weekly-chart-canvas'),plotted,row=>row.merged,row=>row[field],'GitHub Finder weekly average PR '+metric+' time')});outlierNote.textContent=detected.length?(outlierToggle.checked?'Excluded ':'Found ')+detected.length+' metric value'+(detected.length===1?'':'s')+' above the configured hour thresholds.':'No values exceed the configured hour thresholds in the filtered pull requests.';outlierRows.innerHTML=detected.map(item=>'<tr><td><a href="'+e(item.row.url)+'" target="_blank" rel="noreferrer">'+e(item.row.repository+' '+item.row.identifier)+'</a></td><td>'+e(item.row.merged.slice(0,10))+'</td><td>'+e(item.metric[0].toUpperCase()+item.metric.slice(1))+'</td><td>'+e(hours(item.value))+'</td><td>'+(item.excluded?'Excluded':'Included (above threshold)')+'</td></tr>').join('')||'<tr><td colspan="5" class="empty">No values exceed the configured thresholds.</td></tr>'}
function syncMultiLabels(){multis.forEach(control=>{const values=multiLabels(control),total=control.querySelectorAll('input').length,summary=values.length===total?'All '+control.dataset.filterPlural:values.join(', '),output=control.querySelector('[data-multi-label]');output.textContent=values.length?summary:'All '+control.dataset.filterPlural;output.title=values.length?summary:''})}function finderFilterChips(){const names={from:'From',to:'To'},labels=multis.flatMap(control=>{const values=multiLabels(control);return values.length?[control.dataset.filterLabel+': '+(values.length===control.querySelectorAll('input').length?'All '+control.dataset.filterPlural:values.join(', '))]:[]});Object.entries(filters).filter(([key,input])=>key!=='text'&&input.value).forEach(([key,input])=>labels.push((names[key]||key)+': '+input.value));const chips=scope.querySelector('[data-gh-filter-chips]');chips.replaceChildren(...labels.map(label=>{const chip=document.createElement('span');chip.className='filter-chip';chip.textContent=label;chip.title=label;return chip}));scope.querySelector('[data-gh-active-label]').hidden=!labels.length;syncMultiLabels()}
function apply(){const text=filters.text.value.toLowerCase(),from=filters.from.value,to=filters.to.value,type=selected('type'),repository=selected('repository'),state=selected('state'),author=selected('author'),team=selected('team'),teamMapping=selected('teamMapping'),reviewer=selected('reviewer'),jira=selected('jira'),timing=selected('timing');filtered=rows.filter(r=>selectedMatch(type,r.type)&&selectedMatch(repository,r.repository)&&selectedMatch(state,r.state)&&selectedMatch(author,r.author)&&selectedAny(team,r.authorTeams)&&(!teamMapping.size||(teamMapping.has('mapped')&&r.authorTeams.length)||(teamMapping.has('unmapped')&&!r.authorTeams.length))&&selectedAny(reviewer,r.reviewers)&&(!jira.size||(jira.has('linked')&&r.jira.length)||(jira.has('unlinked')&&!r.jira.length))&&(!timing.size||(r.type==='pull_request'&&((timing.has('complete')&&r.pickupHours!==null&&r.reviewHours!==null)||(timing.has('missing_pickup')&&r.pickupHours===null)||(timing.has('missing_review')&&r.reviewHours===null)||(timing.has('missing_both')&&r.pickupHours===null&&r.reviewHours===null))))&&(!from||date(r).slice(0,10)>=from)&&(!to||date(r).slice(0,10)<=to)&&(!text||[r.repository,r.identifier,r.title,r.author,r.authorPerson,...r.authorTeams,r.state,...r.reviewers,...r.prs,...r.jira].join(' ').toLowerCase().includes(text)));filtered.sort((a,b)=>{const av=sortKey==='date'?date(a):a[sortKey],bv=sortKey==='date'?date(b):b[sortKey],c=(typeof av==='number'&&typeof bv==='number')?av-bv:String(av??'').localeCompare(String(bv??''),undefined,{numeric:true,sensitivity:'base'});return sortDir==='asc'?c:-c});page=0;finderFilterChips();finderCharts();draw()}
Object.values(filters).forEach(x=>x.addEventListener(x.tagName==='INPUT'?'input':'change',apply));multis.forEach(control=>{control.addEventListener('change',apply);control.querySelector('[data-multi-all]').onclick=()=>{control.querySelectorAll('input').forEach(input=>input.checked=true);apply()};control.querySelector('[data-multi-none]').onclick=()=>{control.querySelectorAll('input').forEach(input=>input.checked=false);apply()}});outlierToggle.addEventListener('change',finderCharts);scope.querySelector('[data-gh-clear]').onclick=()=>{Object.values(filters).forEach(x=>x.value='');multis.forEach(control=>control.querySelectorAll('input').forEach(input=>input.checked=false));outlierToggle.checked=false;apply()};scope.querySelector('[data-gh-prev]').onclick=()=>{page--;draw()};scope.querySelector('[data-gh-next]').onclick=()=>{page++;draw()};manager.querySelector('[data-column-add]').onclick=()=>{const s=manager.querySelector('[data-column-add-select]');if(s.value){visible.push(s.value);draw()}};apply()});
document.querySelectorAll('[data-gh-outlier-threshold]').forEach(input=>input.addEventListener('change',()=>input.closest('.app-view').querySelector('[data-gh-outlier-toggle]').dispatchEvent(new Event('change'))));
document.querySelectorAll('.github-finder-view').forEach(scope=>deferFinderSearch(scope,'[data-gh-filter="text"]','[data-gh-search]','[data-gh-clear]'));
...
def page(title: str, body: str, generated_at: datetime, snapshot_id: str) -> str:
    ...
    nav = (
        ...
        '<div class="nav-links"><a href="#/">Overview</a>'
        '<a href="#/issue-finder">Issue Finder</a>'
        '<a href="#/github-finder">GitHub Finder</a></div>'
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(title)}</title><style>{CSS}{FINDER_CHART_CSS}</style></head><body><nav>{nav}</nav><main id="top">{body}</main><script>{echarts_js}</script><script>{JS}</script></body></html>'''
...
def main() -> None:
    ...
    finder_pages = (
        '<section class="app-view issue-finder-view" data-route="/issue-finder" data-title="Issue Finder">'
        f'<div class="breadcrumbs">{internal_link("#/", "Overview")} / Issue Finder</div>'
        '<header class="landing-header finder-heading"><div><span class="eyebrow">Jira explorer</span>'
        '<h1>Issue Finder</h1><p class="muted">Find and analyze Jira issues from the pinned team-field query scopes.</p></div>'
        f'<strong class="finder-heading-count">{len(issue_finder_issues):,} issues</strong></header>'
        f'{issue_finder_section(issue_finder_issues, report_teams)}</section>'
        '<section class="app-view github-finder-view" data-route="/github-finder" data-title="GitHub Finder">'
        f'<div class="breadcrumbs">{internal_link("#/", "Overview")} / GitHub Finder</div>'
        '<header class="landing-header finder-heading"><div><span class="eyebrow">GitHub explorer</span>'
        '<h1>GitHub Finder</h1><p class="muted">Find and analyze pull requests and commits from configured repositories.</p></div>'
        f'<strong class="finder-heading-count">{len(github_finder.get("records", [])):,} records</strong></header>'
        f'{github_finder_section(github_finder, people_directory, report_teams)}</section>'
    )
    body = (
        overview
        + finder_pages
